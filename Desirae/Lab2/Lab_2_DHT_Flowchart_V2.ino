/*
  ...
  V2
  Edited 14 March 2022
  by Desirae Minnett
  Note: Changed logical statements to account for precendence and added condition for both temp and humidity in unideal condition
*/
...
int lLimTemp =  -50; // Lower abosolute temperature limit // doesnt really exist for wheat so choose -50
int uLimHumi =  50;  // Upper abosolute humidity limit
int lLimHumi =  40;  // Lower absolute humidity limit
int uiLimTemp = 20;  // Upper ideal temperature limit
int liLimTemp = 18;  // Lower ideal temperature limit
int uiLimHumi = 40;  // Upper ideal humidity limit 
int liLimHumi = 38;  // Lower ideal humidity limit 
...

{
  ...
  if((t < uLimTemp && t > lLimTemp) && (h < uLimHumi && h > lLimHumi))
  {
    ...
    //if temperature is not within ideal range and humidity is not within ideal range
    else
    {
      Serial.println("Temperature and humidity is not in ideal zone");
      Serial.println("*Slow yellow LED*");
      Serial.println("*Slow buzzer activated*");   
    }
  }
