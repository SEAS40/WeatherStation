...
if (h>llhi && h<ulhi) { //GREEN - Within the ideal limits
  ...
  hled_status="Humidity LED: Green";
}
...
if (t>llti && t<ulti) { //GREEN - Within the ideal limits
  ...
  tled_status="Temperature LED: Green";
}
  ...
  tled_status="Temperature LED: Red";
}
else { // BLUE - within acceptable limits
  digitalWrite(green_t,LOW);
  digitalWrite(red_t,LOW);
  ...
  tled_status="Temperature LED: Blue";
}
...
//CO status
if (co==0){
  co_status="CO: Not detected";
}
else {
  co_status="CO: WARNING! CO DETECTED";
}

//Temperature status
if (t>ult){
  t_status="Temperature: WARNING! TEMPERATURE IS TOO HIGH!";
}
else if (t<llt) {
  t_status="Temperature: WARNING! TEMPERATURE IS TOO LOW!";
}
else if(t<llti ||  t>ulti){
  t_status="Temperature: Out of ideal limits!";
}
else {
  t_status="Temperature: OK";
}

//Temperature status
if (h>ulh){
  h_status="Humidity: WARNING! HUMIDITY IS TOO HIGH!";
}
else if (h<llh) {
  h_status="Humidity: WARNING! HUMIDITY IS TOO LOW!";
}
else if(h<llhi ||  h>ulhi){
  h_status="Humidity: Out of ideal limits!";
}
else {
  h_status="Humidity: OK";
}

Serial.println();
Serial.println();
Serial.println("SENSORS STATUS");
Serial.println(co_status);
Serial.println(t_status);
Serial.println(h_status);
Serial.println();
Serial.println("ACTUATORS STATUS"); //not in the diagram, this is just o make it easier to check if the code is running correctly
Serial.println(buzzer_status);
Serial.println(tled_status);
Serial.println(hled_status);
Serial.println();
Serial.println();
